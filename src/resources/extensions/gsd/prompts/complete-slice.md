...
9. Review task summaries for `key_decisions`. Append any significant decisions to `.gsd/DECISIONS.md` if missing. **For each appended decision, also call `capture_thought`** with `category: "architecture"` (or `"pattern"`) so it lands in the memory store.
10. Review task summaries for patterns, gotchas, or non-obvious lessons learned. If any would save future agents from repeating investigation or hitting the same issues, append them to `.gsd/KNOWLEDGE.md`. Only add entries that are genuinely useful — don't pad with obvious observations. **For each appended entry, also call `capture_thought`** with the matching category (`gotcha`, `convention`, `pattern`, `environment`) so the insight is retrievable via `memory_query` in future sessions.
